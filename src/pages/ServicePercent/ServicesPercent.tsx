...
const Category = () => {
  ...
  const { mutate } = useDelete({
    queryKey: "ServicePercent",
    path: "/servicePercent/Delete",
    onSuccess: () => {
      toast.success("1 item deleted", { pauseOnHover: false });

      setTimeout(() => {
        client.invalidateQueries({ queryKey: ["ServicePercent"] });
      }, 1000);
    },
    onError: (error) => {
  ...
  const columns: GridColDef<(typeof allData)[number]>[] = [
    ...
    { field: "servicePerecnt", headerName: "Services Count", flex: 1 },
    {
      flex: 1,
      editable: false,
      field: "serviceName",
    ...
    },
    {
      ...
      renderCell: (row: any) => (
        <Button
          type="button"
          onClick={() =>
            navigate(`/pages/services-percent/edit/${get(row, "id")}`)
          }
        >
          {get(row, "row.edit")}
        </Button>
      ...
      <Container>
        ...
        >
          ...
          <Button
            type="button"
            variant="contained"
            onClick={() => navigate("/pages/services-percent/create")}
          >
            Create
          </Button>
        </Box>
