...
const FaqQuestion = () => {
  ...
  const { mutate } = useDelete({
    queryKey: "FaqQuestion",
    path: "/FaqQuestion/Delete",
    onSuccess: () => {
      toast.success("1 item deleted", { pauseOnHover: false });

      setTimeout(() => {
        client.invalidateQueries({ queryKey: ["FaqQuestion"] });
      }, 1000);
    },
    onError: (error) => {
  ...
  const columns: GridColDef<(typeof allData)[number]>[] = [
    ...
    },
    {
      ...
      renderCell: (row: any) => (
        <Button
          type="button"
          onClick={() => navigate(`/pages/faq-question/edit/${get(row, "id")}`)}
        >
          {get(row, "row.edit")}
        </Button>
      ),
      ...
      <Container>
        ...
        >
          ...
          <Button
            type="button"
            variant="contained"
            onClick={() => navigate("/pages/faq-question/create")}
          >
            Create
          </Button>
        </Box>
