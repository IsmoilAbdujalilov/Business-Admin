...
const TeamMemberEdit = () => {
  ...
  const { mutate } = usePut({
    queryKey: "TeamMember",
    path: "/TeamMember/Update",
    onSuccess: () => {
      toast.success("One value has been updated", { pauseOnHover: false });
  ...
  const teamData = useGet({
    queryKey: "TeamMember",
    path: `TeamMember/GetById?id=${get(paramId, "id", "")}`,
  });
  ...
  useEffect(() => {
    setFullName(get(teamData, "data.content.fullName", ""));
    if (lang) {
      setRoleInputs({
        [lang]: get(teamData, `data.content.role.${lang}`),
      });
    }
  }, [get(teamData, "data")]);
